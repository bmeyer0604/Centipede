...
class Game {
    ...
    setMushrooms() {
        for(let i = 0; i < 25; i++) {
            ...
            let randY = Math.round((Math.random() * (this.DIM_Y * 0.66 - 38) + 45) / 65) * 65;
    ...
    setCentipede() {
        let x = 15;
        for(let i = 0; i < 12; i++) {
            let newSegment = new CentipedeSegment(this.context, this.DIM_X, this.DIM_Y, x, 15, this);

            if(i === 11) newSegment.head = true;
